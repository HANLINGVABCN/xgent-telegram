...
class ProtocolParser:
    ...
    def _collect_marked_body(
        ...
        ``smart_match=True`` 时，如果精确匹配找不到，会扫描以 ``AGENT_END_``
        ...
        begin_nonce = marker[len("AGENT_END_"):]
        threshold = (
        ...
        while i < len(lines):
            ...
            # 智能匹配：记录最相似的 AGENT_END_<nonce> 行
            if smart_match:
                stripped = lines[i].rstrip()
                if stripped.startswith("AGENT_END_"):
                    end_nonce = stripped[len("AGENT_END_"):]
    ...
    def has_unclosed_block(cls, ai_response: str) -> bool:
        ...
        while i < len(lines):
            ...
            _body, end_i, _sm, _sim = cls._collect_marked_body(lines, i + 1, end_marker)
            if end_i is None:
                return True
            i = end_i + 1
        return False
