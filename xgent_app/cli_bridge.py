...
class CliBot:
    ...
    async def edit_message_text(self, text: str, chat_id: Optional[int] = None,
        ...
        重绘分两档（cli_render.TerminalScreen）：块整块可见时上移整块重画；
        块比屏幕高时走增量续写——流式编辑带的是累计全文，只把新增的尾部
        写出去，最后一行保持可改写，长回复不会把终端刷成同一消息的 N 份
        递增拷贝。

        重绘不可行的情况：中间夹了别的输出（这条已经不是屏幕最后一块）、
        正文在中途被改写得增量接不上、或者输出不是 tty（重定向到文件）。
        这几种情况下宁可多打一份也不能上移光标去擦不属于自己的内容。
