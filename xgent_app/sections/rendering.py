...
async def _notify_cli_generation_started(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> None:
    """CLI 会话开始生成时，落一条占位记录，让跨端观察者立刻镜像到 Telegram。

    CliBot.send_message 只把"流式输出中..."画到终端屏幕，不落库——三种响应
    函数（流式/后台流式/非流式）开头发的那条占位消息，在 CLI 会话下只有
    本地终端看得到。服务端跨端观察者（_web_external_record_watcher）靠轮询
    global_messages 表发现新记录才会把内容镜像到 Telegram；不落这一行的话，
    Telegram 端要一直等到最终回复落库（往往几秒到几十秒之后）才有动静，
    体感上就是"CLI 说话，Telegram 半天没反应，不是立即同步"。
...
async def send_streaming_response(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    try:
        ...
        await _notify_cli_generation_started(context, chat_id)
