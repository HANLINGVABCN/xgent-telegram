...
def create_attachment_reference(
    ...
    try:
        ...
        if expected_binary and mime_type:
            # readx 读取时已用 native_binary_kind 确认该 mime 可原生直传，故信任调用方
            # 登记为 binary；序列化层再按当前模型能力决定发送或降级为文本占位。
            reference.update(kind="binary", mime_type=mime_type)
        else:
...
def _restore_reference(reference: dict, upload_root: str | Path,
    ...
    if kind == "text":
        ...
        # 原生二进制（pdf/音频/视频）：这是“恢复的历史附件”，当前模型不支持该 mime 时应
        # 降级成文本占位而非硬失败（区别于用户显式媒体输入——那种必须报错、不能静默丢弃）。
        # 故打上 degrade_if_unsupported 标记，只让序列化层对这类部件降级。
        parts.append({
            "type": "binary", "mime_type": mime_type,
            "filename": name,
            "data": base64.b64encode(data).decode("ascii"),
            "degrade_if_unsupported": True,
        })
    else:
