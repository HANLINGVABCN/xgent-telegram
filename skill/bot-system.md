...
- `stdin`：向已有 shell 会话输入终端宏；推荐用 `text:` / `key:` 按行分开普通文本和按键，`+` 表示同一拍组合键，空格或多行 `key:` 表示顺序按键；兼容旧 inline 宏语法，但复杂输入应优先行式语法；完整语法见 `skill/stdin-syntax.md`。
...
- `file:`：创建或覆盖服务器文件；只把写入结果回灌给 AI，不回灌文件本体。
