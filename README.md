...
默认指令以信息保留为优先，不限定摘要字数。恢复记录按当前目标与要求、最近对话与停点、事实与执行结果、未完成事项、文件与回看入口、其他背景组织，要求保留用户修正和禁止事项、具体参数及路径，并明确区分建议、已执行、已验证、失败或停止。已有恢复记录中的有效信息必须与新增记录合并，不能因没有再次提及而遗漏；未读取的附件原件和未持久化的 `read-x` 正文不得补造。
