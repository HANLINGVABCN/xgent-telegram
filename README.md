...
AI：已创建后台任务：cron 每天 03:00 清理 /var/log 下 7 天前的
...
- **后台任务调度**：`trigger` 命令（AI 用 run-x 调用）支持延迟执行、定时任务（cron）、条件触发和持续监控，命令无时长限制，到点自动执行并把结果推回对话。
...
- **全量附件上下文**：每轮直接携带当前未清空对话的全部图片原件和文本全文，不受普通消息历史深度或 Agent 开关影响。
...
- 装好的组件一律是绿的；`web` 那行后面另外写着两个开关的当前状态，例如
  `🟢 网页 开 · 🔴 终端 关 · http://127.0.0.1:8790`——开关关着不算异常
- 可以多选，按 `1 → 2 → 3` 的顺序依次安装，过程中按需询问 Web 密码、端口、Bot Token 和用户 ID
- 已安装的组件再选一次，进入的是**配置界面**（改密码/端口、开关网页对话与网页终端、换 Token、换授权 ID、移除组件）
- 三个组件互相独立：只装 `cli` 就能在终端里用，不需要 Web 密码，也不需要 Telegram
- Web 的密码、端口和两个开关在 `install.sh`、`xgent` 终端、网页里改都一样：
  改动落进同一份数据库，正在运行的服务几秒内自己跟上，不用重启
...
Telegram 照片、以文件发送的图片及 Web 上传统一保存原件，并在 SQLite 上传记录中保存附件关联、真实类型、完整附言和顺序。聊天模型及 `media-x` 生成的图片也保存原图并建立关联。每次对话请求发出前，程序自动把所有仍有关联的图片作为图片输入、文本文件作为全文输入交给模型。流式、非流式和 Agent 后续调用使用同一逻辑，不需要用户操作或模型发出读取指令；重启后仍有效。

- `/depth` 只控制普通历史消息，不会裁掉附件。同一附件关联在每次请求中只注入一次，相册按 Telegram 消息顺序排列。
- 首版支持文本文件和图片。文本支持 UTF-8、带 BOM 的 UTF-16/32、GB18030，保留全文及末尾内容；没有原来的 12,000 字符截断。图片按实际字节识别格式，不转换或压缩，具体格式还须获得所选模型支持。Telegram 照片使用 Telegram 提供的最大版本；需要保留上传前的原始分辨率时，应以文件发送。
- 每轮输入成本和请求体积随累计附件增加。没有近期附件数量上限、按需读取或自动摘要替代。只有用户明确执行压缩或清空记忆，旧附件才退出自动上下文。平台是否提供缓存及如何收费，以该平台为准。
- 容量已知时检查完整输入并预留输出空间；文本按 UTF-8 字节、图片按尺寸进行保守 token 估算，不是供应商的精确分词计数，可能提前拒绝请求。容量未知时发送完整请求，以接口反馈为准。
- 超出上下文、图片数量或请求体限制，原件缺失或变更，文本无法完整解析，或者模型不支持图片时，明确报错并停止本轮，不会自动删减附件继续回答。解析失败的上传关联会保留并阻止后续不完整请求。
- 可核验原件路径的旧上传记录会自动补充关联；无原件或无可信路径的旧记录会明确报错。旧版本已截断的附言无法恢复，只能保留旧索引中的部分；文件原件仍在时，文本全文和图片原件可以恢复。
- `/clear_memory` 解除所有附件、压缩摘要和归档链关联并取消尚未结束的相册，不主动删除磁盘原件。聊天展示和 `/export` 仍只含附件索引，不写入附件全文或图片 Base64；迁移实例时应同时备份数据库和 `xgent_storage/`。
- 提供商配置、提示词、黑名单等配置导入不属于对话附件，不会把配置文件自动交给模型。

### 模型容量配置

模型列表接口提供的容量和图片能力元数据会自动保存；不会仅凭模型名称猜测容量。也可以通过 `BotMemoryDB.set_config("model_request_limits", value)` 写入 SQLite `config` 表，并重启服务加载。`value` 按 `提供商名称/模型ID` 设置，下面的数值仅为格式示例，应替换为所用接口的真实限制：
...
各字段均可省略；未知限制不要填 `null` 或猜测值。另支持 `max_input_tokens`（独立输入上限）和 `image_token_budget`（每张图的保守估算值）。已知容量时默认预留最多 4096 输出 token，并受已知输出上限约束；思考参数抬高输出预算后会再次按最终请求检查。配置表示服务端限制，只用于拒绝无法完整发送的请求，不用于裁剪内容。

`media-x` 使用默认媒体模型对应的配置，不沿用聊天模型的容量或图片能力。另支持 `max_files`（本次显式输入位置数，可为 0，重复路径也计数）与 `max_file_bytes`（单份输入原件的字节上限）。原生 PDF、音频、视频等文件无法可靠换算 token 时，只预检可估算的文字/图片部分与输出预算，原生文件的实际容量交接口校验；不会把忽略二进制的估算说成完整容量保证。请求体上限仍按包含全部原件 Base64 的最终请求检查。

## 上下文压缩与归档

Telegram、Web、CLI 共用 `/compress`，菜单中也有「压缩上下文」按钮。流程是**先导出、再清空、临时读取归档、生成一条普通 AI 回复**，不执行回复中的 Agent 协议。Agent 关闭也能压缩；已有对话正在执行时拒绝插入压缩。

- 压缩与 `/export` 使用同一个导出服务，保存完整持久化记录，不受 `/depth` 限制。确认 ZIP 和文本文件落盘、校验快照与清空代次之后，才在事务中登记恢复任务并清除旧对话、镜像和附件关联。
- 恢复当轮复用 `read-x` 的文件读取与临时上下文构造，只读取本次导出的最新 `Na` 全局记忆，并在末尾追加一次归档时冻结的压缩指令。普通系统提示词为空，不额外提供人设、全局追加提示词、外部记忆、技能、Agent 协议、`Nb` 附件清单或归档说明，也不读取旧附件原件和其他历史段。导出文件仍照常保留这些归档类别，但不会因此发送给压缩模型。旧任务重试同样遵守此输入范围。
- 恢复回复通过现有前台流式、后台流式或非流式通路显示，并作为一条普通 AI 回复保存。它受历史深度限制，**不再额外置顶摘要**；后续请求仅固定保留一个简短的 ZIP、文本目录及最新原文路径入口，需要旧正文时可通过 `read-x` 回看。
- **一旦导出并清空完成，旧附件就退出自动上下文，即使之后恢复失败。** 原件仍在磁盘；新上传附件和新生成图片继续按现有机制提供。
- 导出失败、清空前停止或快照发生并发变化时不清空。清空后接口失败、超时、停止、空结果或明确输出截断时，不自动恢复旧对话；保留归档入口和失败状态。完整请求超限时停止，不自动截断、拆分或减少内容。
- 「重试恢复」复用原归档和原指令，不再次导出或清空，也不覆盖新增聊天。停止时保留已经生成的部分并标明未完成，不将其登记为有效压缩结果。重启不会自动重试收费请求。
- 从导出开始，Telegram 持续显示 typing，Web 头像显示输出状态；刷新和重连后恢复真实状态。文件或回复投递失败不撤销已经保存的结果，Web 历史中的下载入口仍可使用。
- 单独清空记忆会取消恢复任务、解除归档链和固定路径，下一次压缩从第 1 段重新编号，不删除磁盘文件。旧式隐藏摘要会幂等转换为一条普通 AI 回复，旧 ZIP 不改写。

压缩指令位于 `prompts/compression.txt`，在「提示词」菜单中名为「上下文压缩提示词」，支持查看、修改、下载和从文件重载。菜单保存立即生效；直接编辑磁盘文件后需重载。已有自定义文件不会被启动初始化覆盖。

默认指令将输出定义为详细交接记录和状态快照，以独立信息点为单位，不限定字数或压缩比例。恢复记录按当前目标与要求、最近对话与停点、事实与执行结果、未完成事项、文件与回看入口、其他背景组织，要求保留用户修正和禁止事项、具体参数及路径，并明确区分建议、已执行、已验证、失败或停止。提供两组仅用于示范的记录粒度对照，要求逐项核对原文，不能用主题名替代多个要求，也不能把示例写成真实记忆。已有恢复记录中的有效信息必须与新增记录合并，不能因没有再次提及而遗漏；未读取的附件原件、未持久化的 `read-x` 正文和未显示的内部思考不得补造。提示词与模拟请求测试不保证真实模型的信息保留效果，仍需用实际对话核对遗漏。
压缩结果使用客观、正式的书面语言，忽略历史中的角色扮演语气、称呼、口癖和表情，但保留用户的实际要求。普通聊天的人设、记忆和技能加载不变。

修改提示词只影响之后新建的压缩任务；已有归档及「重试恢复」仍使用当时冻结的指令。更新时若选择保留 `prompts/`，现有提示词也会保留，需要单独更新压缩提示词才能使用新版。

归档保存在 `xgent_storage/exports/日期/唯一目录/`，ZIP 中每个文本文件在同目录都有可直接读取的版本。导出只保留以下文件，例如完成两次压缩后：

```text
拦截记录.txt
提示词.txt              导出时当前有效的对话提示词
压缩提示词.txt          当次实际使用的压缩指令
1a全局记忆.txt          第一段完整持久化记录
1b附件路径.txt          第一段附件路径、类型、顺序及已有校验信息
1c压缩结果.txt          第一次完成的恢复回复
2a全局记忆.txt          第二段记录，开头包含第一次恢复回复
2b附件路径.txt
2c压缩结果.txt
3a全局记忆.txt          当前尚未压缩的记录
3b附件路径.txt
```

`Nc` 只在该段有完整恢复结果时创建。`1c` 的内容同时作为第二段开头的一条真实回复出现在 `2a` 中，这是有意保留；同一个 `Na` 不额外复制摘要。压缩前生成的 ZIP 保持不变，本次新结果在下一次导出中加入对应 `Nc`。任务状态、各段原始消息及结构化元数据保留在数据库，不再额外导出镜像 JSON、逐轮提示词或 manifest。

ZIP 不复制附件原件，也不嵌入请求中的图片 Base64。摘要不能保证语义绝对无损，因此始终保存原文；归档只含已持久化内容，无法补回过去未入库的 `read-x` 正文。`read-x` 的实现和跨轮持久化行为未改变。
...
| `/stream` | 开关流式输出 |
| `/skills` | 管理公有/私有 Skill 的启用、普通关闭与彻底隐藏 |
...
- `compression.txt`：只在压缩请求末尾追加的上下文压缩指令。
...
5. `skill-public/` 和 `skill-private/` 文件索引（普通关闭保留名字和路径，彻底隐藏不计入）；
6. 不依赖技能开关的简短技能查阅规则；
7. Agent 关闭时的限制说明。

`skill-public/`、`skill-private/` 只以索引形式进入提示词，模型需要时才读取具体文件。Skill 文件简介使用 `!` 围栏协议块；同一文件中的多个 `!` 块会按出现顺序合并：
...
- 每个 skill 默认启用，保留简介与路径。Telegram 的 `/skills` 和 Web 设置每项只有一个三态按钮，点击按绿色「启用」、黄色「已关闭」、红色「已隐藏」循环切换。
- 普通关闭只保留 `xxx.md: 当前技能已关闭（路径: …）`，需要时模型仍可按现有 `read-x` 协议查阅开头的 `!` 简介块及正文；全部普通关闭也保留查阅入口。
- 彻底隐藏不注入该技能条目；红色状态再点击一次恢复启用。它不是文件访问权限控制，也不会删除历史聊天中已出现的名字。Agent 关闭时仍禁止执行读取协议。
- 普通关闭保存在 `disabled_skills`，彻底隐藏保存在 `hidden_skills`；旧禁用配置转为普通关闭，默认隐藏列表为空，重启保留状态。
...
| `skill-loader` | 按需查阅普通关闭技能的简介及完整用法 |
| `stdin-syntax` | `stdin-x` 协议的详细用法 |
| `trigger-x-protocol` | `trigger` 后台任务命令完整文档（子命令、调度类型、条件表达式语法） |
...
- `shell-x`：创建长驻、持续输出和交互式命令的会话；配套 `stdin-x`（写入输入，空 body 或只写 `wait:` 即纯读新增输出）、`shellkill-x`（结束会话）；
- `intel-x`：代码语义分析——定位定义/引用/调用方、取符号源码、项目骨架图等（`context`/`callers`/`map`/`summary` 等子命令）；
- `read-x`：读取服务器文件；
...
- `fetch-x`：抓取指定网页内容；
- `media-x`：调用默认媒体模型生成内容，可用 `file:` 与 `prompt:` 提供有序的本地参考文件；
- `over-x`：本回合收尾。放在回复末尾，正文是预写的收尾语。系统照常执行前面的协议、把结果照常发给用户并记入上下文，但不再把结果回传给模型，省掉一轮空回复。只对 `run-x`、`edit-x`、`shellkill-x`、`file-x` 生效，且要求它们全部成功——出现别的协议或任何一个失败，`over-x` 自动作废，结果照常回传。

此外，**后台任务**不再是独立协议，而是 AI 用 `run-x` 调用的 `trigger` 命令：`trigger add --cmd "..." --after 30s|--cron "..."|--when "..."` 登记延迟/定时/周期/条件监控任务，`trigger show`/`trigger kill <id>` 管理。命令无时长限制，结果到点自动推回对话。
...
纯生成兼容原有写法，也可以用 `prompt:` 显式给出指令：

```media-x
<<BEGIN_gen_garden_1577
Please generate an image of a quiet autumn garden.
<<END_gen_garden_1577
```
...
```media-x
<<BEGIN_edit_lollipop_7321
file: /data/source.png
prompt: 把图中人物手里的棒棒糖换成冰淇淋，其他内容保持不变。
<<END_edit_lollipop_7321
```

多文件参考：

```media-x
<<BEGIN_edit_reference_1577
file: /data/source.png
file: /data/icecream.png
prompt: 把第一张图中人物手里的棒棒糖换成第二张图里的冰淇淋。
其他内容保持不变。
<<END_edit_reference_1577
```

路径必须换成**运行服务器上已存在的普通文件的绝对路径**，不限于上传目录，只能读取进程本身有权限访问的文件。支持中文、空格、成对外层引号，以及 Windows 路径，例如 `file: "C:\images\参考 图.png"`；不能用浏览器所在电脑的路径代替服务器路径。不接受 URL、相对路径、通配符或目录扫描。

- 首个非空行是 `file:` 或 `prompt:` 时启用新格式。`prompt:` 必填，`file:` 可省略或重复，必须全部放在 `prompt:` 前。字段名周围可有空白，冒号兼容 `:` 和 `：`。
- `prompt:` 后的全部内容原样作为多行指令，不再解析其中的字段或协议。文件顺序与声明一致，同一路径重复出现也不会去重。
- 后端完整读取指定原件并构造模型输入，无需模型先执行 `read-x`。图片识别真实格式并校验，文本完整解码，其余文件使用原生二进制输入。源文件不覆盖、不转码、不截图替代、不截断，也不继承 `read-x` 的 8 MB/512 KB 限制。
- 请求只带本次指定文件、生成指令和媒体生成说明，不带聊天历史、人设或其他已关联附件。文件内容属于参考资料，不是系统指令。

| 接口格式 | 可表达的参考输入 |
| --- | --- |
| OpenAI / OpenAI 兼容 | 图片、全文文本、PDF、WAV、MP3 |
| Claude | 图片、全文文本、PDF |
| Gemini / Vertex | 图片、全文文本，以及原生二进制文件（含模型支持的文档、音频、视频） |

以上是接口表达范围，不是对模型能力的保证；模型或兼容网关不支持某种输入时会保留接口错误。仍使用现有对话端点，不切换图片编辑端点，也不新增文件上传服务。任一文件缺失、不可读、图片损坏、类型不支持或已知限制超出时，在调用前明确失败并报告文件序号、路径与原因，不会发送删减后的请求。输入文件不会新增普通聊天附件关联，原先已关联的附件不变；生成图片仍先保存再投递，继续进入后续上下文，Web 图文合并和下载行为不变。

默认协议说明来自 `prompts/agent_addon.txt`；部署时保留了自定义提示词的，需要在提示词菜单更新或从该文件重载。自动测试验证原始字节、全文和顺序到达实际适配层，不保证真实模型能精确完成编辑；“棒棒糖换冰淇淋”的效果需用支持图片编辑的媒体模型实测。
...
每条菜单单独保存最终正文、格式和按钮。菜单内跳转、技能切换和返回都更新原消息，刷新、重连或重启后仍可使用；长中文路径的按钮动作也持久保存。过期修订、已删除对象或失效确认流程会提示重新打开，历史恢复不会执行旧点击。清空及压缩切换会解除旧菜单关联，旧事件不能复活菜单。旧版未保存的菜单无法凭点击日志还原，需要重新打开一次。

菜单界面记录不进入模型上下文或全局记忆导出。Web 不显示按钮点击及可识别的内部设置流水，但原记录仍保留；普通用户命令、实际回复、错误、下载和压缩重试入口继续显示。

AI 生成的媒体在同一个 XGent 消息块中展示：图片（多图按生成顺序）、完整文字、原件路径提示。实时输出和刷新恢复使用相同的分组标识，图片投递期间刷新也不会拆成文字块与图片块；每张图仍可预览和下载原件。
...
Cloudflare 临时隧道（`*.trycloudflare.com`）[不支持 SSE](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/trycloudflare/)。网页通过这类地址访问时自动使用短连接增量事件；其他地址优先使用 SSE，连接未就绪或心跳中断时自动切换。增量事件带游标，可以补接进度、去重并同步多个标签页，不会重新执行命令，也不影响后端的模型流式输出。断线过久或服务重启时会重新同步持久化历史；无需反复手动刷新。
...
默认关闭——它等于放开任意命令执行。开关在两处，写的是同一个配置：`install.sh` → `1` → `2 web` 里的「开启网页终端」，或 `/web` 菜单里的「🖥 终端」。它和网页对话各自独立开关，共用同一个端口和同一份访问密码。
...
│  ├─ attachments.py          # 附件持久化引用、旧记录迁移与完整上下文恢复
│  ├─ context_limits.py       # 完整请求容量检查，不裁剪附件
...
├─ skill-public/             # 公有 Skill 说明及配套脚本
├─ skill-private/            # 私有 Skill（不进仓库、不被更新覆盖）
├─ tests/                     # 单元测试
├─ tools/                     # 开发检查工具
└─ docs/
...
node tools/web_transport_smoke.mjs
node tools/web_menu_smoke.mjs
```
...
菜单回归另外使用真实菜单处理器和隔离 SQLite 数据库，覆盖刷新、双窗口、重启、长中文按钮、删除与清空后的旧事件重放；截图写入 `workspace/web-menu-qa/`。手动测试菜单可运行 `python tools/web_menu_fixture.py --serve`，不会调用实际模型或 Telegram。
