...
压缩指令位于 `prompts/compression.txt`，在「提示词」菜单中名为「上下文压缩提示词」，支持查看、修改、下载和从文件重载。菜单保存立即生效；直接编辑磁盘文件后需重载。已有自定义文件不会被启动初始化覆盖。默认内容为：

> 压缩前文所有内容，必须不丢失任何行为，意图，进度，用户需求和工作方向，用户和ai最后几轮对话着重。
