...
Telegram、Web、CLI 共用 `/compress`，菜单中也有「压缩上下文」按钮。压缩使用当前对话模型，在后台独立请求一次，不执行返回文本中的 Agent 协议。已有对话正在执行时拒绝插入压缩；压缩期间可以停止。

- 首次提交当前完整持久化对话和全部有效图片、文本附件，不受 `/depth` 限制。再次压缩只提交上次摘要及本段新增记录、附件，不反复提交已归档的旧原文。
- 完整请求超限、附件无法恢复、提示词为空或不可读、接口错误、空摘要及明确输出截断都会停止压缩，不拆分、不删减后重试。
- 摘要生成后，先保存原文、结构化记录、当次实际指令、摘要、附件索引及 ZIP，校验归档后再以数据库事务切换上下文。停止、清空、并发写入或归档失败时不会用旧快照替换当前记忆。
- 成功后等待用户下一条消息。后续请求前部固定携带一次最新摘要，以及 ZIP、最新原文 TXT 和归档清单的绝对路径，跨历史窗口、Agent 开关和重启保留。清单列出全部已归档段落，不需要先解压 ZIP 才能读取原文。
- **成功压缩后，旧附件不再自动进入请求。** 原件仍在磁盘，路径不等于原图或全文已在模型输入中。新上传和新生成的图片继续按全量附件机制提供。
- 文件投递失败不撤销已经保存的压缩结果，Web 历史中的下载入口和服务器归档路径仍可使用。单独清空记忆会解除整条归档链，下一次压缩从第 1 段重新编号，不删除磁盘文件。
...
归档保存在 `xgent_storage/compressions/日期/唯一目录/`，同目录保留可读取的文本及 JSON。后续 `/export` 保留原有导出文件，并按压缩链顺序追加，例如完成两次压缩后：

```text
全局记忆1.txt / 全局记忆1.json    第一段原始记录
压缩提示词2.txt                   第一次摘要
全局记忆3.txt / 全局记忆3.json    第二段原始记录
压缩提示词4.txt                   第二次摘要
全局记忆5.txt / 全局记忆5.json    当前未压缩记录
压缩指令_第1次.txt                当次实际指令，后续次数同样保存
附件索引_第1段.json               路径、类型、顺序、校验及原有元数据
manifest.json                    归档清单
```

这里的「压缩提示词」按约定指压缩结果，实际指令另存。ZIP 不复制附件原件，也不嵌入请求中的图片 Base64。摘要不能保证语义绝对无损，因此始终保存原文；归档只含已持久化内容，无法补回过去未入库的 `read-x` 正文。`read-x` 的实现和跨轮持久化行为未改变。
