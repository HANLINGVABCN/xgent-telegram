...
Telegram 照片、以文件发送的图片及 Web 上传统一保存原件，并在 SQLite 上传记录中保存附件关联、真实类型、完整附言和顺序。每次对话请求发出前，程序自动把所有仍有关联的图片作为图片输入、文本文件作为全文输入交给模型。流式、非流式和 Agent 后续调用使用同一逻辑，不需要用户操作或模型发出读取指令；重启后仍有效。
...
- 每轮输入成本和请求体积随累计附件增加。没有近期附件数量上限、按需读取或摘要替代。平台是否提供缓存及如何收费，以该平台为准。
...
- `/clear_memory` 解除所有附件关联并取消尚未结束的相册，不主动删除磁盘原件。聊天展示和 `/export` 仍只含附件索引，不写入附件全文或图片 Base64；迁移实例时应同时备份数据库和 `xgent_storage/uploads/`。
...
5. `skill-public/` 和 `skill-private/` 文件索引（被禁用的 skill 不计入）；
6. Agent 关闭时的限制说明。
