...
class AttachmentTests(unittest.TestCase):
    ...
    def test_audio_video_are_invalid_but_always_keep_their_path(self):
        # 硬规则：上传口默认读不了音视频，但绝不能凭空消失——引用必留 path，
        # 还原出的占位块必带 Original:<路径>，让 AI 仍能用工具定位原件。
        # （read 留存走 expected_binary 显式登记为 binary，见下一个用例。）
        samples = (
            (b"ID3\x04\x00\x00\x00\x00\x00\x00\xff\xfb\x90\x00" + bytes(64),
             "voice.mp3", "audio/mpeg"),
            (b"\x00\x00\x00\x18ftypmp42\x00\x00\x00\x00mp42isom\x00\x00\x00\x08mdat",
             "clip.mp4", "video/mp4"),
            (b"OggS\x00\x02" + bytes(64), "note.ogg", "audio/ogg"),
        )
        for data, name, mime in samples:
            with self.subTest(name=name):
                ref = self.reference(data, name, mime_type=mime)
                self.assertEqual("invalid", ref["kind"])
                self.assertTrue(ref["path"])  # 引用层保留了相对路径
                parts, _, errors = self.restore(ref)
                self.assertFalse(errors)
                self.assertTrue(parts)
                self.assertEqual("text", parts[0]["type"])
                self.assertIn("Original:", parts[0]["text"])
                self.assertIn(name, parts[0]["text"])

    def test_native_binary_opt_in_persists_as_binary(self):
        # read 留存登记 pdf/音频/视频 时显式 expected_binary=True：读取时已用
        # native_binary_kind 确认可原生直传，故登记为 binary（而非上传口默认的 invalid）。
        # 仍保留 path，还原出可供模型消费的 binary 部件（含原始字节）。
        samples = (
            (b"%PDF-1.7 fake", "doc.pdf", "application/pdf"),
            (b"ID3\x04\x00\x00\x00\x00\x00\x00\xff\xfb\x90\x00" + bytes(64),
             "voice.mp3", "audio/mpeg"),
            (b"\x00\x00\x00\x18ftypmp42\x00\x00\x00\x00mp42isom\x00\x00\x00\x08mdat",
             "clip.mp4", "video/mp4"),
        ...
        for data, name, mime in samples:
            with self.subTest(name=name):
                ref = self.reference(data, name, mime_type=mime, expected_binary=True)
                self.assertEqual("binary", ref["kind"])
                self.assertEqual(mime, ref["mime_type"])
                ...
                self.assertEqual("text", parts[0]["type"])
                self.assertIn("Original:", parts[0]["text"])
                binary_parts = [p for p in parts if p.get("type") == "binary"]
                self.assertTrue(binary_parts)
                self.assertEqual(data, base64.b64decode(binary_parts[0]["data"]))
